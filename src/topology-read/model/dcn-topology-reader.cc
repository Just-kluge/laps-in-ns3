...
NodeContainer DcnTopologyReader::Read (void) {
  ...
  if ( !topgen.is_open () ) {
      std::cerr <<"Dcn topology file " << GetFileName() << " object is not open, check file name and permissions" << std::endl;
      return m_nodes;
    }
  ...
  NS_LOG_INFO ("Dcn topology should have " << totnode << " nodes ");
    // std::cout <<"Dcn topology should have " << totnode << " nodes " << std::endl;
